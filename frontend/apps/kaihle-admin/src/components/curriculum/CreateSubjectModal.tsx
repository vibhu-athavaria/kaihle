/**
 * CreateSubjectModal - Modal for creating a new subject.
 *
 * Fields: Name (required, max 100), Code (required, max 20, uppercase hint),
 * Description (optional), Icon (Lucide icon name), Color (hex), Active toggle
 */
...
// Brand palette colors from design spec
const BRAND_COLORS = [
...
export function CreateSubjectModal({
  ...
  const validate = (): boolean => {
    ...
    if (color && !/^#[0-9A-Fa-f]{6}$/.test(color)) {
      errors.color = "Color must be a valid hex code (e.g., #1a5c38)";
    }
  ...
  return (
    <Modal
      ...
      description="Add a new subject to the platform."
      maxWidth="md"
    >
      <form onSubmit={handleSubmit} className="space-y-4 mt-4">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">
            Unique identifier, uppercase (e.g., MATH, SCI, ENG)
          </p>
          {fieldErrors.code && (
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              placeholder="e.g., BookOpen, Calculator, Flask"
          ...
          <p className="mt-1 text-xs text-gray-500">
            Lucide icon name (e.g., BookOpen, Calculator, Atom, Palette)
          </p>
        </div>
