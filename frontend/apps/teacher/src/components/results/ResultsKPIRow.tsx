...
function KPICard({ label, children, ariaLabel }: KPICardProps) {
  return (
    ...
    >
      <div className="font-sans text-xs uppercase tracking-[0.08em] text-gray-400 mb-1">
        {label}
      </div>
      {children}
    </div>
...
export function ResultsKPIRow({
  ...
  if (submittedCount === 0) {
    return (
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        >
          <div className="font-display text-3xl leading-none text-brand-ink">
            0
          </div>
        ...
        >
          <div className="font-display text-3xl leading-none text-brand-muted">
            —
          </div>
          <div className="text-xs text-brand-muted mt-1">
        ...
        >
          <div className="font-display text-3xl leading-none text-gray-400">
            0
          </div>
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      >
        <div
          className={`font-display text-3xl leading-none ${avgStyle.textClass}`}
        >
      ...
      >
        <div className="font-display text-3xl leading-none text-brand-ink truncate">
          {highest?.studentName ?? "—"}
        </div>
        {highest && (
      ...
      >
        <div
          className={`font-display text-3xl leading-none ${
            needsAttentionCount > 0 ? "text-brand-red" : "text-gray-400"
          }`}
        >
          {needsAttentionCount}
        </div>
