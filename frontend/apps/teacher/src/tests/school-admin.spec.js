...
test.describe("School Admin UI", () => {
    test.beforeEach(async ({ page }) => {
        await page.addInitScript(() => {
            window.localStorage.setItem("auth_user", JSON.stringify({ id: "user-1", role: "SCHOOL_ADMIN", school_id: "school-1" }));
    ...
    test("Role tabs switch between Teachers/Students/Parents", async ({ page }) => {
        await page.goto("/school/users");
        await expect(page.locator("text=Teachers")).toBeVisible();
    ...
    test("Teacher role accessing school routes redirects to teacher dashboard", async ({ page }) => {
        await page.addInitScript(() => {
            window.localStorage.setItem("auth_user", JSON.stringify({ id: "user-1", role: "TEACHER", school_id: "school-1" }));
        });
        await page.goto("/school/overview");
...
test.describe("CreateClassModal curriculum suggestions", () => {
    ...
    test("Grade 7 suggests Cambridge Lower Secondary", async ({ page }) => {
        ...
        await expect(page.locator("text=Suggested: Cambridge Lower Secondary")).toBeVisible();
    });
});
