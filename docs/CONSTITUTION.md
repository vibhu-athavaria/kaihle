...
## 9. Git and Task Workflow

**Branch naming format:** `M{N}-{E}-T{N}_{type}/{short-description}`
- `{type}` = `feature` | `fix` | `chore` | `migration`
- `{short-description}` = lowercase, hyphen-separated
- Example: `M3-1-T8_feat/teacher-content-browser`
- Branch MUST match the task ID being implemented.

**Doubt filing:** If blocked by an architectural, design, or curriculum doubt, file `docs/doubts/DOUBT-{timestamp}.md` with: persona routing, category, blocking status, options considered. Maximum one doubt per task.

---

## 10. Task Completion Checklist — MANDATORY

Before declaring any task done, run through every item. Do not self-assess — verify each one against the actual output.

### Every task
- [ ] All acceptance criteria met and verified, not assumed
- [ ] Tests written and passing (`uv run pytest` / `jest`)
- [ ] Linters clean (`ruff`, `mypy`, `prettier`, `tsc`)
- [ ] No inline imports — all imports at file top
- [ ] No hardcoded secrets, model names, or magic strings
- [ ] No `# type: ignore` added

### Backend changes
- [ ] Schema verified against both ORM model AND `kaihle_v2_1_schema.sql` — conflicts surfaced, not guessed
- [ ] Every new table/column has `school_id` (unless curriculum table)
- [ ] All queries filter by `school_id` (or explicit KaihleAdmin bypass)
- [ ] All LLM calls go through `router.py` — no direct SDK imports
- [ ] Migration included with model changes — autogenerated, not hand-written
- [ ] Migration is idempotent (uses `IF NOT EXISTS` / `IF EXISTS` where needed)
- [ ] Celery tasks verify complete postconditions, not just presence

### Frontend changes
- [ ] Correct app for the role — no cross-role code in `apps/`
- [ ] Shared components used from `packages/ui` — no custom Modal, Skeleton, EmptyState
- [ ] Mastery colors via `getMasteryStyle()` — never inlined
- [ ] Loading states: skeletons for page load, spinners for buttons, pulsing badge for background generation
- [ ] Every list has an explicit empty state
- [ ] All modals use `Modal` from `packages/ui` — focus trap guaranteed

### Content/document changes
- [ ] If content was moved between documents — confirm every displaced piece has a verified destination
- [ ] CONSTITUTION updated if any locked rule or project convention changed
- [ ] BRV curated if feature implementation changed
...
| Doubt files (blocked tasks) | `docs/doubts/DOUBT-{timestamp}.md` |
