...
**Layout wrapper:** `StudentLayout` — no sidebar, top nav tabs (desktop), bottom nav (mobile)  
**Action color:** Green `#1a5c38`  
**Heading font:** Fraunces · Body: Nunito  
**Page background:** `#f9fafb`  
**Mobile-first:** design at 375px, comfortable at md:
...
Top nav bar with 4 horizontal tabs + active green underline:
```
Home (/student/dashboard) · Progress (/student/my-progress) · Study plans (/student/study-plans) · Assessments (/student/assessments)
```

### Mobile (below md:)
Bottom nav bar with 4 icon+label items — `md:hidden`:
```
Home (House icon) · Progress (BarChart icon) · Study (BookOpen icon) · Assessments (Clipboard icon)
```
Active state: `text-brand-primary` — no background fill on mobile items.  
Settings is NOT in the nav — accessed via avatar/profile icon in topbar right.
...
**Layout:** `OnboardingLayout` — full screen, NOT wrapped in `OnboardingRoute` guard (infinite loop risk)
...
2. Subject score cards (3 cards, `grid-cols-3`)
3. My classes (class cards, locked/unlocked per Gate 2)
4. What's waiting for you (next step cards, priority order)

### Subject score cards
- Left-border color from mastery band: green/amber/red/gray
- Score: large font, mastery color
- Label: "Strong" / "Developing" / "Needs work" / "Not assessed"
- Mini progress bar showing assessed subtopics count
- `score=null` → "–" not "0%"

### Class cards (Gate 2)
DiagnosticStatus drives rendering:
- `PENDING/IN_PROGRESS` → locked: `opacity-60`, footer "Complete diagnostic to unlock" → amber CTA "Start diagnostic →"  
- `COMPLETED` → unlocked: mastery shown, footer "View class →" green

### Next step card priority order (max 3)
...
| Class cards + lock state | `GET /api/v1/onboarding/status` → `diagnostics_by_class` |
...
### Layout
- Subject tabs below topnav (derive from enrolled classes, show most-recently-active first)
...
**Bottom nav:** Study plans tab active

### Layout
- Plans grouped by subject (section label per subject)
- Within each subject: ACTIVE first, GENERATING second, COMPLETED last

### Plan card states
| Status | Visual | Actions |
|---|---|---|
| ACTIVE | Standard card, green dot | "Start →" green button |
| GENERATING | Amber pulsing dot, "Preparing your plan..." | No action yet |
...
**Layout:** Full-screen assessment mode — no nav tabs, no bottom nav
...
### Layout
- Full page, no bottom nav active (settings is reached via avatar/profile icon)
...
| Update name | `PATCH /api/v1/users/me` (if endpoint exists) |
...
| Rule | Detail |
|---|---|
| No sidebar | `StudentLayout` only — no `DashboardLayout`. No exceptions. |
| Never construct student ID in URLs | Always use `/me` shortcut (`/students/me/gap-map` not `/students/{id}/gap-map`) |
