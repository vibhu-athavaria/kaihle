...
def upgrade() -> None:
    ...
    assessment_type = postgresql.ENUM(
        "DIAGNOSTIC", "TOPIC_SPECIFIC", "PROGRESS_CHECK", "FINAL", name="assessment_type", create_type=False
    )
    assessment_type.create(op.get_bind(), checkfirst=True)

    # assessment_status
    assessment_status = postgresql.ENUM("DRAFT", "ACTIVE", "CLOSED", name="assessment_status", create_type=False)
    assessment_status.create(op.get_bind(), checkfirst=True)

    # attempt_status
    attempt_status = postgresql.ENUM(
        "NOT_STARTED", "IN_PROGRESS", "COMPLETED", "ABANDONED", name="attempt_status", create_type=False
    )
    attempt_status.create(op.get_bind(), checkfirst=True)

    # question_type
    question_type = postgresql.ENUM("MCQ", "TRUE_FALSE", "SHORT_ANSWER", name="question_type", create_type=False)
    question_type.create(op.get_bind(), checkfirst=True)

    # scored_by
    scored_by = postgresql.ENUM("RULE", "LLM", "PENDING", name="scored_by", create_type=False)
    scored_by.create(op.get_bind(), checkfirst=True)

    # study_plan_status
    study_plan_status = postgresql.ENUM(
        "GENERATING", "ACTIVE", "COMPLETED", "ABANDONED", name="study_plan_status", create_type=False
    )
    study_plan_status.create(op.get_bind(), checkfirst=True)

    # resource_type
    resource_type = postgresql.ENUM("VIDEO", "ARTICLE", "INTERACTIVE", name="resource_type", create_type=False)
    resource_type.create(op.get_bind(), checkfirst=True)

    # lesson_plan_status
    lesson_plan_status = postgresql.ENUM(
        "GENERATED", "EDITED", "USED", "ARCHIVED", name="lesson_plan_status", create_type=False
    )
    lesson_plan_status.create(op.get_bind(), checkfirst=True)

    # subscription_tier
    subscription_tier = postgresql.ENUM(
        "TRIAL", "STARTER", "GROWTH", "SCALE", name="subscription_tier", create_type=False
    )
    subscription_tier.create(op.get_bind(), checkfirst=True)

    # subscription_status
    subscription_status = postgresql.ENUM(
        "ACTIVE", "PAST_DUE", "CANCELLED", "EXPIRED", name="subscription_status", create_type=False
    )
    ...
    user_role = postgresql.ENUM(
        "STUDENT", "TEACHER", "SCHOOL_ADMIN", "PARENT", "KAIHLE_ADMIN", name="user_role", create_type=False
    )
    user_role.create(op.get_bind(), checkfirst=True)

    # auth_token_type
    auth_token_type = postgresql.ENUM("MAGIC_LINK", "REFRESH", name="auth_token_type", create_type=False)
    auth_token_type.create(op.get_bind(), checkfirst=True)

    # onboarding_status
    onboarding_status = postgresql.ENUM(
        "PENDING", "IN_PROGRESS", "COMPLETED", name="onboarding_status", create_type=False
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "difficulty_level IS NULL OR difficulty_level BETWEEN 1 AND 5", name="chk_subtopic_difficulty"
        ),
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "difficulty_level IS NULL OR difficulty_level BETWEEN 1.0 AND 5.0", name="chk_qb_difficulty"
        ),
    ...
    op.create_index(
        "idx_school_curricula_one_primary",
        "school_curricula",
        ["school_id"],
        unique=True,
    ...
    op.create_index(
        "idx_responses_scored", "student_responses", ["scored_by"], postgresql_where=sa.text("scored_by = 'PENDING'")
    )
