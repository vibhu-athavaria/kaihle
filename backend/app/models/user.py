...
class User(Base):
    ...
    student_profile = relationship("Student", back_populates="user", foreign_keys="Student.user_id")

class Student(Base):
    __tablename__ = "students"
    
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=True)  # For student users
