...
def get_progress_by_student_and_week(db: Session, student_id: int, week_start: datetime) -> Optional[Progress]:
...
def update_progress(db: Session, progress_id: int, progress_update: ProgressUpdate) -> Optional[Progress]:
...
def get_student_progress_history(db: Session, student_id: int, limit: int = 10) -> List[Progress]:
    return db.query(Progress).filter(
        Progress.student_id == student_id
    ).order_by(desc(Progress.week_start)).limit(limit).all()

def get_student_total_points(db: Session, student_id: int) -> int:
    result = db.query(func.sum(Progress.points_earned)).filter(
        Progress.student_id == student_id
    ).scalar()
    return result or 0

def get_student_current_streak(db: Session, student_id: int) -> int:
...
def get_student_total_lessons(db: Session, student_id: int) -> int:
...
def award_badge_to_student(db: Session, student_id: int, badge_id: int) -> StudentBadge:
...
def check_and_award_badges(db: Session, student_id: int):
